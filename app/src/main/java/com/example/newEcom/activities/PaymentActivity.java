...
public class PaymentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        nameEditText = findViewById(R.id.card_no_edt);
        emailEditText = findViewById(R.id.name_on_card_edt);
        phoneEditText = findViewById(R.id.card_expire_edt);
        addressEditText = findViewById(R.id.cvv_edt);
...
//                                                    }
                ...
                public void onCallback(boolean adequateStock) {
                    if (!adequateStock){
                        ...
                        stockErrorTextView.setVisibility(View.VISIBLE);
                        Toast.makeText(PaymentActivity.this, "One of the products has got less stock left :(", Toast.LENGTH_SHORT).show();
                    }
                    else {
                        Log.i("check","2");
                        changeToFirebase();
...
//        void onCallback(String docid, int oldstock, int quan);
    ...
    private boolean validate() {
        ...
        if (nameEditText.getText().toString().trim().length() == 0) {
            nameEditText.setError("Card Number is required");
            isValid = false;
        }
        ...
        if (phoneEditText.getText().toString().trim().length() == 0) {
            phoneEditText.setError("Expiration is required");
            isValid = false;
        }

        if (addressEditText.getText().toString().trim().length() == 0) {
            addressEditText.setError("CVV is required");
            isValid = false;
        }
        return isValid;
    }
